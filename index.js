...
const { getAllProducts, getProduct, setProductUrl, getProductChunks } = require('./downloads');
const {
...
client.on('interactionCreate', async interaction => {
    ...
    modal.addComponents(
      ...
      new ActionRowBuilder().addComponents(
        new TextInputBuilder()
          ...
          .setPlaceholder('https://uhservices.gg/download')
          .setStyle(TextInputStyle.Short)
          .setRequired(false)
          .setMaxLength(500)
      ),
  ...
  if (interaction.isModalSubmit() && interaction.customId === 'update_modal') {
    ...
    let downloadUrl = interaction.fields.getTextInputValue('download_url').trim() || '';
...
🔗 ${url}`,
  ...
  if (interaction.isChatInputCommand() && interaction.commandName === 'setupdownloads') {
    ...
    const dlChannel = interaction.guild.channels.cache.find(c => c.name === 'downloads') || interaction.channel;

    const embed = new EmbedBuilder()
      .setTitle('📦  PRODUCT DOWNLOADS')
      .setDescription(
        '> Select your product from the dropdown below and click **⬇️ DOWNLOAD** to get your file.
...
Select your product below:',
  ...
  if (interaction.isStringSelectMenu() && ['dl_page_1','dl_page_2','dl_page_3'].includes(interaction.customId)) {
    ...
    if (product.url) {
      ...
      embed.setDescription('⏳ Download link not yet available for this product.
