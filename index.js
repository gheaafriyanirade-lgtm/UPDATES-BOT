...
client.on('interactionCreate', async interaction => {
  ...
  if (interaction.isStringSelectMenu() && interaction.customId === 'select_update_type') {
    ...
    const isTimeExtension = typeKey === 'time_extension';
  ...
  if (interaction.isModalSubmit() && interaction.customId === 'update_modal') {
    ...
    if (statusTransRaw && typeKey !== 'time_extension') {
      // Support →, ->, >, to, /
    ...
    if (typeKey === 'time_extension' && statusTransRaw) {
    ...
    // Pick status indicator emoji based on new status
    let statusDot = '⚪';
    if (newStatus) {
      const statusKey = Object.keys(STATUS_TYPES).find(k => STATUS_TYPES[k] === newStatus);
      if (statusKey === 'updated')  statusDot = '🟢';
      if (statusKey === 'updating') statusDot = '🔵';
      if (statusKey === 'testing')  statusDot = '🟡';
    } else {
      // Fall back to update type
      if (typeKey === 'detected')  statusDot = '🔴';
      if (typeKey === 'undetected') statusDot = '🟢';
      if (typeKey === 'disabled')   statusDot = '🔴';
      if (typeKey === 'enabled')    statusDot = '🟢';
      if (typeKey === 'maintenance') statusDot = '🟡';
      if (typeKey === 'updating')   statusDot = '🔵';
    }

    const embedTitle = `${statusDot} ${baseTitle}`;
