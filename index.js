...
// ─── Pending update store (product + type while modal is open) ───────────────
const pendingUpdates = {};

// ─── Interactions ─────────────────────────────────────────────────────────────
client.on('interactionCreate', async interaction => {

  // ── Step 1: /postupdate → show type select menu ──────────────────────────
  ...
  if (interaction.isStringSelectMenu() && interaction.customId === 'select_update_type') {
    ...
    // Store selected type for this user
    ...
    const notesInput = new TextInputBuilder()
      .setCustomId('notes')
      .setLabel('NOTES (separate lines with |)')
  ...
  if (interaction.isModalSubmit() && interaction.customId === 'update_modal') {
    ...
    try {
      ...
      await interaction.reply({
        content: `✅ Update posted to <#${targetChannel.id}>`,
        ephemeral: true
      });
    } catch (err) {
      await interaction.reply({
