...
const commands = [
  ...
  new SlashCommandBuilder()
    ...
    .setDescription('Post a status update to #status-updates'),
...
🔗 ${url}`,
      ...
      .addFields(
        ...
        },
        {
          ...
          value: [
            ...
            '`/statusupdate` — Post a status update to #status-updates',
  ...
  // ── /setstatus → open status update modal ────────────────────────────────────
  if (interaction.isChatInputCommand() && interaction.commandName === 'statusupdate') {
    ...
    modal.addComponents(
      new ActionRowBuilder().addComponents(
        new TextInputBuilder()
          .setCustomId('ss_product')
          .setLabel('PRODUCT NAME')
          .setPlaceholder('e.g. FORTNITE - ANCIENT')
          .setStyle(TextInputStyle.Short)
          .setRequired(true)
          .setMaxLength(100)
      ),
      new ActionRowBuilder().addComponents(
        new TextInputBuilder()
          .setCustomId('ss_status')
          .setLabel('STATUS  (e.g. updated -> updating)')
          .setPlaceholder('e.g. updated -> updating')
          .setStyle(TextInputStyle.Short)
          .setRequired(true)
          .setMaxLength(40)
      ),
      new ActionRowBuilder().addComponents(
        new TextInputBuilder()
          .setCustomId('ss_notes')
          .setLabel('NOTES  (optional, separate with |)')
          .setPlaceholder('e.g. Fixed injection | Keys unfrozen')
          .setStyle(TextInputStyle.Paragraph)
          .setRequired(false)
          .setMaxLength(1000)
      ),
  ...
  // ── setstatus_modal submit ────────────────────────────────────────────────────
  if (interaction.isModalSubmit() && interaction.customId === 'setstatus_modal') {
    ...
    let oldStatus = null;
    let newStatus = null;
    const parts = statusRaw.split(/→|->|>|\bto\b|\//).map(p => p.trim());
    if (parts.length === 2) {
      oldStatus = STATUS_TYPES[parts[0]] || null;
      newStatus = STATUS_TYPES[parts[1]] || null;
    } else if (parts.length === 1) {
      ...
      const lastKey = productLastStatus[product.toLowerCase()];
      const lastStatus = lastKey ? STATUS_TYPES[lastKey] : null;
      if (lastStatus && lastStatus !== newStatus) oldStatus = lastStatus;
    }
    ...
    const embedColor = getProductColor(product);
    const fields = [
      { name: 'Product', value: `\`${product.toUpperCase()}\``, inline: false },
    ];

    if (oldStatus && newStatus) {
      fields.push(
        { name: 'Changed from', value: `${oldStatus.emoji}  ${oldStatus.label}`, inline: false },
        { name: 'New Status',   value: `${newStatus.emoji}  ${newStatus.label}`, inline: false },
      );
    } else if (newStatus) {
      fields.push({ name: 'New Status', value: `${newStatus.emoji}  ${newStatus.label}`, inline: false });
    }
    ...
    // Find #status-updates channel
    const statusChannel = interaction.guild.channels.cache.find(
      c => c.name.toLowerCase().replace(/[^a-z0-9]/g, '') === 'statusupdates' &&
           c.type === ChannelType.GuildText
    ) || interaction.channel;
    ...
    if (pingStr) {
      ...
      if (clean === 'everyone') pingText = '@everyone';
        ...
        const roleMatch = pingStr.match(/\d+/);
        if (roleMatch) pingText = `<@&${roleMatch[0]}>`;
        else {
          const role = interaction.guild.roles.cache.find(r => r.name.toLowerCase() === clean);
          if (role) pingText = `<@&${role.id}>`;
        }
      }
    }
    ...
    try {
      await statusChannel.send({ content: pingText || null, embeds: [embed] });
      await interaction.reply({
        content: `✅ Status update posted to <#${statusChannel.id}>`,
        flags: 64,
      });
      autoDelete(interaction, 5_000);
    } catch (err) {
