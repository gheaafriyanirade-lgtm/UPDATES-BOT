...
client.on('interactionCreate', async interaction => {
  ...
  if (interaction.isModalSubmit() && interaction.customId === 'update_modal') {
    ...
    const statusTypes = ['status_change', 'update', 'patch', 'undetected', 'detected', 'disabled', 'enabled', 'maintenance', 'updating'];
    const shouldParseStatus = statusTypes.includes(typeKey);

    if (statusTransRaw && shouldParseStatus) {
      // Support →, ->, >, to, /
